...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
	 ...
	 var email = p.email ? '<span>mail</span><a href="mailto:' + p.email +
          '">' + p.email + '</a>' : '';

      var twitter = p.twitter ? '<span>twitter</span>' +
          '<a href="http://twitter.com/' + p.twitter + '">' + p.twitter + '</a>' : '';

     var www = p.www ? '<span>www</span><a href="' + p.www +'">' + p.www + '</a>' : '';
	 var blog = p.blog ? '<span>blog</span><a href="' + p.blog + '">' + p.blog + '</a>' : '';

     var github = p.github ? '<span>github</span><a href="' + p.github +'">' + p.github + '</a>' : '';
     var phone = p.phone ? '<span>phone</span><a href="' + p.phone + '">'+ p.phone +'</a>' : '';

	 var html2 = [];
	 var presenter_data = [email, www, blog, twitter, github, gplus, phone].filter(function(n){return n});
      var html2 = presenter_data.join('<br>');
